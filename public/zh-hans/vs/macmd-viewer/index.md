...
- **预览主题：**MacMD Viewer 内置 12 种文稿主题。MarsDawn 有四种：Dawn、Classic、Modern 和 Vivid，各有浅色与深色。
- **图表与数学式：**两者都能画出 Mermaid 图表、也都有代码上色。MarsDawn 还能排版 KaTeX 数学式；MacMD Viewer 自己的介绍页没有提到数学式排版。
- **Finder 整合：**两者都有 Finder 的快速查看扩展，对 `.md` 文件按空格键就能看到排好版的页面。
- **PDF 与打印：**两者都能把排好版的页面导出或打印成 PDF。
- **系统要求：**MacMD Viewer 需要 macOS 14（Sonoma）以上。MarsDawn 需要 macOS 26（Tahoe）以上。
...
- **价格：**MacMD Viewer 一台 Mac 一次 USD 19.99（三台的组合包和大量授权更贵）。MarsDawn 免费下载，之后以 USD 4.99 一次解锁。
...
MarsDawn 即将在 Mac App Store 上架，现在还没开卖。在那之前，免费的 `marsdawn` 命令行工具今天就能把任何 Markdown 文件转成 PDF，Mermaid 图表和代码上色都在，而且不需要安装其他东西：
...
- 和在 VS Code、浏览器或 Claude Desktop 看 Markdown 比较：[比较一下](/zh-hans/vs/markdown-preview-tools/)。
...
- [日本語](https://marsdawn.southern-light.dev/ja/vs/macmd-viewer/index.md): MacMD Viewer は Markdown を読み取り専用で表示、USD 19.99。MarsDawn は編集とプレビューを並べて表示、Mac App Store で無料お試し後に USD 4.99 を一度だけ。
