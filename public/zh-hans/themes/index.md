...
- ![Dawn 主题的并排版面：左边是 Markdown 源代码，右边是排版后的页面。](/assets/screens/01-split-800.png) **Dawn（默认）**
- ![Classic 主题，预览占满整个窗口。](/assets/screens/02-classic-800.png) **Classic（典雅）**
- ![Vivid 主题的并排版面。](/assets/screens/04-vivid-800.png) **Vivid（活泼）**
- ![MarsDawn 的深色模式，并排版面。](/assets/screens/03-dark-800.png) **深色模式**

Modern 还没有截图；第四张是深色模式。

- **黎明**，默认主题：和这个网站一样的暖色纸感与 Mars Rust 强调色。
- **典雅**：比较朴素、像纸质文稿的配色。
- **流行**：比较冷调、当代感的配色。
- **活泼**：比较明亮、对比度较高的配色。

每种主题都有各自的浅色和深色版本，所以切换 Mac 的外观，连带切换的是主题本身的配色，不只是界面的颜色。

## PDF 导出和打印用同一个主题

导出成 PDF 或打印，用的是你主题的浅色配色：Mermaid 图表会直接画进去，代码块保留语法上色，分页时也会尽量不让标题和内容分开，或切开表格与图表。免费的 [marsdawn 命令行工具](/zh-hans/cli/)使用同一套输出程序，所以脚本或 agent 也能用 `--theme` 生成一模一样的 PDF，四种主题都可以。
...
- 完整的 PDF 导出步骤，从命令行开始：[Markdown 转 PDF](/zh-hans/markdown-to-pdf/)。
- MarsDawn 现在还做不到的事：[这份清单](/zh-hans/limits/)。
- 把导出的 PDF 交给不写 Markdown 的人：[分享 PDF](/zh-hans/sharing-exported-pdfs/)。
