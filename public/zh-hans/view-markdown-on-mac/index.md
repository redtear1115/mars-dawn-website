...
`.md` 文件是纯文本。标题、粗体、表格和图表，都是用记号写成的：`#` 代表标题，`**` 包住粗体，竖线符号画出表格，`mermaid` 代码块则是一张图。用纯文本编辑器打开，看到的就是这些记号。想照作者的意思读到排好的页面，就需要有东西把它排版出来。

## 现在就能用，而且免费：转成 PDF

免费的 `marsdawn` 命令行工具，能把 Markdown 文件排版成 PDF，任何一台 Mac 都打得开。表格、数学式、Mermaid 图表和代码上色都会排好，而且不需要安装其他东西，连 MarsDawn app 都不用。
...
- 预览会随着你打字即时更新，两边的窗格一起滚动。
- Mermaid 流程图和时序图直接画在预览里，代码块也会上色。
...
- 改用 VS Code、浏览器或 Claude Desktop 看 Markdown：[比较一下](/zh-hans/vs/markdown-preview-tools/)。
...
- [日本語](https://marsdawn.southern-light.dev/ja/view-markdown-on-mac/index.md): .md ファイルは、整形用の記号が入ったプレーンテキストです。Mac 上でレンダリングされた見た目を読む方法：今すぐ使える無料の marsdawn コマンドラインツールで PDF にする方法と、Mac App Store に近日公開予定の MarsDawn アプリで読む方法。
