...
| `2` | 入力ファイルが見つからない | パスとファイル名を確認する |
| `3` | MarsDawn がインストールされていない（`open` のみ） | アプリをインストールするか、アプリが不要な `export` を使う |
| `4` | 出力する PDF がすでにある | `--force` で上書きするか、`-o` で別の場所に書き出す |
| `5` | 書き出しに失敗した | JSON の結果の `message` を読む |
| `64` | 使用方法のエラー。範囲外の行や、複数ファイルに対する `--line` の指定などを含む | オプションや値を直す。このエラーは `--json` を付けても stderr にテキストで出力される |
