...
MarsDawn 可以免费下载。开始 14 天试用后，所有功能都能使用；试用结束后想继续使用，花 USD 4.99 解锁一次就好。没有订阅，也不需要账号。

![MarsDawn 使用 Vivid 主题，左边是 Markdown 源代码，右边是排版后的页面。](https://marsdawn.southern-light.dev/assets/screens/04-vivid-1180.png)
...
- 在 Mac App Store 免费下载 MarsDawn。
- 开始试用后，14 天内所有功能都能使用：所有主题与布局、PDF 导出与打印、快速查看，以及 Siri 和快捷指令动作。
- 试用结束后想继续使用，花 USD 4.99 解锁一次就好。这是 App 内购买，不是订阅，不会自动续费，之后也不会再扣款。
- 试用本身也不会扣款。试用结束时，除非你选择解锁，否则不会购买任何东西。
- 不需要账号，MarsDawn 从不要求你创建账号。

## 如果没有解锁

- 14 天后，在你解锁之前，无法在 MarsDawn 中阅读、编辑、导出或打印文稿。文稿仍会打开，但内容会被遮住。
- 你的文件不会有任何改变。它们就是你 Mac 上的普通文件，在 Finder 中用“快速查看”依然看得到。
- 免费的 [`marsdawn` 命令行工具](/zh-hans/cli/)不受试用影响，依然能把它们导出成 PDF。
- 如果试用结束时有文稿正打开在 MarsDawn 里，你输入的文字不会丢失，可以用“文件”▸“另存为⋯”保存。
...
- [日本語](https://marsdawn.southern-light.dev/ja/pay-once/index.md): MarsDawn は無料でダウンロードできます。14 日間すべての機能を試したあとは、USD 4.99 を一度だけ支払えば使い続けられます。サブスクリプションもアカウントも不要です。
