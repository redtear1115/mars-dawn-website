# Markdown 仕様書（PRD）テンプレート

エージェントが埋められて、あなたが一度で読み通せる仕様書。課題、目標、要件、フロー図、受け入れ基準。要件をひとつ変えたら、残りを合わせるようエージェントに頼みます。

## テンプレート

[spec.md をダウンロード](/ja/templates/spec/spec.md)
...
![spec.md を marsdawn export で書き出した PDF の 1 ページ目。](/assets/templates/spec-ja.png)

`marsdawn export spec.md` で書き出したもの。無料のコマンドラインツールは MarsDawn のプレビューと同じ見た目で、図もそのまま描きます。

## エージェントに頼む

```
https://marsdawn.southern-light.dev/ja/templates/spec/spec.md のテンプレートを使って、［機能］の仕様を spec.md に書いてください。要件にはすべて ID を付け、フローと受け入れ基準でも同じ ID を使ってください。書き終えたら marsdawn open spec.md を実行してください。
```

## PDF で共有する

`marsdawn export spec.md` を実行すると、隣に spec.pdf ができます。Markdown を読まない人に渡せます。

## できないこと

MarsDawn は仕様、表、図を表示します。受け入れ基準がすべての要件を網羅しているかは確かめません。それはエージェントの仕事で、あなたが読んで確かめることです。

## よくある質問

### フロー図のために何かインストールが必要ですか？

いいえ。MarsDawn と `marsdawn export` は Mermaid を自分で描きます。オフラインでも描けます。

### MarsDawn で仕様を編集できますか？

はい。ソースの隣にプレビューがある Markdown エディタです。エージェントは次にファイルを読むときに、あなたの編集を目にします。

## ほかのテンプレート

- [フローチャート](/ja/templates/flowchart/)：Mermaid の図と、その下に書き出した各ステップ。
- [議事録](/ja/templates/meeting-notes/)：決定事項とアクションアイテム。それぞれに担当者。
