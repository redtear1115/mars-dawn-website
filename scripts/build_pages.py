...
<p>It exports Markdown to PDF without the app. Once the app is out, <code>marsdawn open</code> lets your agent open files in it for you.</p>
...
<pre><code>{_INSTALL}</code></pre>
<p>它不需要 app 就能把 Markdown 輸出成 PDF。app 上架之後，agent 也能用 <code>marsdawn open</code> 在 MarsDawn 裡幫你打開檔案。</p>
...
<p>Chase is writing about tooling for people building agent frameworks &#8212; LangGraph and LangSmith, by name &#8212; not about a person reading a finished document. But his spectrum gives a useful way to size up what you're about to read before you start: the more agentic the system that produced a file, the less you should expect its steps to be predictable from the prompt alone, and the more the file in front of you is worth treating as a record of what actually happened rather than what was supposed to happen.
His &#8220;observe what is going on inside&#8221; is about a running system's internals &#8212; traces (a recorded log of everything the agent did during one run), intermediate steps, tool calls &#8212; not about reading a Markdown plan after the fact. But the underlying reason he gives, that the exact steps taken may not be known ahead of time, applies just as well to the document an agent hands you when it's done: if the steps weren't predictable going in, the report coming out is the only place left to check them.</p>
