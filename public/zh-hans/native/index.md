...
- 两侧同步滚动，正在编辑的段落一直在眼前。
- 编辑器内置 Markdown 语法上色，颜色与预览主题一致。
- 原生窗口、标签页、自动保存和版本历史。
- 快速查看：在 Finder 选中 Markdown 文件按空格键就能预览，图表也会显示。
- Siri 和快捷指令：用模板新建文稿、在笔记收件箱加上一行，或重新打开最近的文稿。
- 支持英文和繁体中文。
...
- [日本語](https://marsdawn.southern-light.dev/ja/native/index.md): 本物の Mac アプリとしての Markdown エディタ：ネイティブのウインドウとタブ、自動保存、バージョン履歴、Finder のクイックルック、Mac らしく動くテキストエディタ。
