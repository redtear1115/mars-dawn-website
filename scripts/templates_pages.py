...
LABELS = {
    ...
    "zh-hans": {"template": "模板", "download": "下载 {file}", "looks": "导出效果",
                "ask": "交给 agent", "share": "导出 PDF 分享", "doesnt": "做不到的事",
                "faq": "常见问题", "more": "其他模板", "sep": "：",
                "img_alt": "{file} 用 marsdawn export 导出的 PDF 第一页。"},
...
PAGES = {
    ...
    "zh-hans": {
        "spec": {
            ...
            "caption": "用 <code>marsdawn export spec.md</code> 导出。免费的命令行工具排版和 MarsDawn 的预览一样，图表也一起画出来。",
        ...
        "flowchart": {
            ...
            "description": "Markdown 的 Mermaid 流程图模板，图的下方把步骤写出来。在 Mac 上预览，也能导出成 PDF。",
            "h1": "Markdown 流程图模板",
            "lede": "一张 Mermaid 流程图，下方把每个步骤写清楚，图和文字可以互相对照。去掉一个步骤，再请 agent 把其他部分改好。",
            "caption": "用 <code>marsdawn export flowchart.md</code> 导出。免费的命令行工具排版和 MarsDawn 的预览一样，图表也一起画出来。",
        ...
        "meeting-notes": {
            ...
            "caption": "用 <code>marsdawn export meeting-notes.md</code> 导出。免费的命令行工具排版和 MarsDawn 的预览一样。",
            "prompt": "用 {url} 这份模板，把这场会议整理进 meeting-notes.md。先写决议，一项一行；每个行动项都要有一位负责人和日期。写好之后运行 marsdawn open meeting-notes.md。",
            "share": "<code>marsdawn export meeting-notes.md</code> 会导出一份 PDF，可以附在会后的邮件里。",
