...
PAGES = {
    ("en", "index"): {
        ...
        "description": "A native Mac Markdown editor with live preview, Mermaid diagrams and PDF export, built for reading what AI agents write. On the Mac App Store.",
        "intro": """
...
""",
    ...
    ("zh-hant", "index"): {
        ...
        "description": "原生的 Mac Markdown 編輯器，有即時預覽、Mermaid 圖表和 PDF 輸出，為讀 AI agent 寫的 Markdown 而做。已在 Mac App Store 上架。",
        "intro": """
<section class="intro hero">
...
<ul>
  ...
  <li><code>open</code>：在 MarsDawn app 中開啟一或多個 Markdown 檔，讓人檢閱，也可以指定每個檔案要定位的行，或在視窗的側邊欄顯示一個資料夾。</li>
</ul>
...
def main() -> None:
    if "PLACEHOLDER" in LISTING_URL:
        print(f"WARNING: LISTING_URL is still {LISTING_URL}: set the Apple ID before this goes live.", file=sys.stderr)
    pages = all_pages()
