# Markdown spec (PRD) template

A spec your agent can fill in and you can read in one sitting: the problem, goals, requirements, a flow diagram and acceptance criteria. When you change a requirement, ask the agent to bring the rest in line.

## The template

[Download spec.md](/templates/spec/spec.md)
...
![The first page of spec.md, exported as a PDF with marsdawn export.](/assets/templates/spec-en.png)

Exported with `marsdawn export spec.md`. The free command-line tool renders like MarsDawn's preview, diagram included.
...
`marsdawn export spec.md` writes spec.pdf beside it, for someone who doesn't read Markdown.

## What it doesn't do

MarsDawn shows the spec, the table and the diagram. It doesn't check that the acceptance criteria cover every requirement. That's the agent's job, and your read.

## Questions

### Does the flow diagram need anything installed?

No. MarsDawn and `marsdawn export` draw Mermaid themselves, offline.

### Can I edit the spec in MarsDawn?

Yes. It's a Markdown editor with the preview beside the source. The agent sees your edit the next time it reads the file.
