...
- The CSP in `public/_headers` allows the site's own scripts (the consent banner, `consent.js`), Google Tag Manager, and the two hashed inline scripts in each page's `<head>` that set Consent Mode defaults and load GTM. Everything else is same-origin stylesheets and images and system fonts only. No web fonts, inline styles, forms, frames or other third-party resources. Anything that needs one of these must change the CSP deliberately.
...
4. **Static, private and fast.** The pages are static and work without scripts. The one script-driven part is Google Analytics 4 through Google Tag Manager, and it starts denied under Consent Mode: no analytics cookie or identifier until the visitor chooses *Accept*, with the choice changeable from every footer. The privacy page says exactly this, and it must keep saying it accurately. The app itself collects nothing.
