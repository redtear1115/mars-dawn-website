...
<ol class="loop-steps">
  ...
  <li><strong>14 日間、すべて使える。</strong> トライアルを始めると、14日間は MarsDawn のすべての機能が使えます。すべてのテーマとレイアウト、PDF 書き出しとプリント、Siri とショートカットのアクションです。Finder のクイックルックはトライアルの有無にかかわらず使えます。</li>
...
<h2>試用期間が終わったら</h2>
<p>14日間のトライアルが終わってロックを解除しなければ、MarsDawn で文書を読んだり編集したり書き出したりプリントしたりできません。内容が覆われた状態で開きます。ファイルはそのまま残り、クイックルックでは引き続き表示され、無料のコマンドラインツールも引き続き書き出せます。<a href="/ja/pay-once/">トライアルとロック解除のページ</a>に、3つの段階を並べた表があります。</p>
""",
    }
