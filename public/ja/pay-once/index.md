...
MarsDawn は無料でダウンロードできます。14日間のトライアルを始めればすべての機能が使えます。その後も使い続けるには、USD 4.99 の一度きりの購入でロックが解除されます。サブスクリプションもアカウントもありません。

![MarsDawn がビビッドテーマで、左に Markdown のソース、右にレンダリングされたページを表示している。](https://marsdawn.southern-light.dev/assets/screens/04-vivid-1180.png)

このスクリーンショットの内容：

1. エディタの Markdown ハイライトも含まれます。
2. すべてのテーマとレイアウトが含まれます。
3. Mermaid 図も含まれます。
4. コードのハイライトも含まれます。
...
2. **14 日間、すべて使える。** トライアルを始めると、14日間は MarsDawn のすべての機能（すべてのテーマとレイアウト、PDF の書き出しとプリント、Siri とショートカットのアクション）が使えます。Finder のクイックルックは、トライアルの有無にかかわらず使えます。
...
- アカウントはありません。MarsDawn がアカウント作成を求めることは決してありません。

## 各段階でできること

|  | トライアル期間（1〜14日目） | トライアル終了、ロック未解除 | ロック解除済み |
|---|---|---|---|
| MarsDawn で文書を開く | できる | 開くが、内容は覆われる | できる |
| MarsDawn で読む・編集する（ソース、プレビュー、Mermaid、数式） | できる | できない | できる |
| MarsDawn から PDF に書き出す・プリントする | できる | できない | できる |
| 入力した文字を「ファイル」▸「別名で保存…」で保存する | できる | できる（トライアル終了時に開いていたウインドウのみ） | できる |
| Siri とショートカットのアクション | できる | できない | できる |
| Finder のクイックルック（Mermaid 図と数式つき） | できる | できる（変わらない） | できる |
| `marsdawn export`（無料のコマンドラインツール）：図と数式つきの PDF | できる | できる（変わらない） | できる |
| Mac 上のファイル | 保存したときのまま | 保存したときのまま（ロックによって変更されることはない） | 保存したときのまま |

トライアルを始める前は、MarsDawn がトライアルの案内を表示します。開始しても料金はかかりません。

トライアル終了後は、MarsDawn で開いた PDF ファイルも同じようにロックされます。
