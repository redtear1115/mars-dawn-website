...
- `--background` は marsdawn 0.5.1 で追加されました。
...
オプトインの [Claude Code フック](https://code.claude.com/docs/en/hooks)です。Claude が Markdown ファイルを書き込んだり編集したりすると、そのファイルを MarsDawn でバックグラウンドで開きます。開くのはセッションごと、ファイルごとに一度だけです。頼んでいないウィンドウは注意をそらすので、追加しない限り有効にならず、プロジェクトごとに追加します。シェルコマンドを実行するだけなので、モデルのトークンは使いません。
...
- `--background` により MarsDawn は前面に出ません。作業中のウィンドウのフォーカスはそのままです。
